...
import { authKeys, messageKeys, threadKeys, channelKeys } from '../queryKeys';
...
export function useThreadMessages(parentMessageId: string | undefined) {
  return useInfiniteQuery({
    queryKey: threadKeys.detail(parentMessageId!),
    queryFn: ({ pageParam }) =>
...
export function useSendThreadReply(parentMessageId: string, channelId: string) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: threadKeys.subscription(parentMessageId) });
...
export function useDeleteLinkPreview() {
  ...
  return useMutation({
    ...
    onMutate: async (messageId) => {
      ...
      const previousData = queryClient.getQueriesData({ queryKey: messageKeys.all });
